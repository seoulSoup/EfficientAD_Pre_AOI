...
SAVE_CONTOUR    = True               # 불량 위치 컨투어 표시 여부
CONTOUR_THRESHOLD = 0.5              # 컨투어 추출용 이진화 임계값 (0~1)
# ─────────────────────────────────────────────
...
def draw_defect_contour(image: np.ndarray, anomaly_map: np.ndarray, threshold: float = 0.5) -> tuple:
    """
    Anomaly Map에서 임계값 이상 영역을 contour로 표시
    - 외곽선: 빨간 실선
    - 내부 fill: 반투명 빨간색 오버레이
    - 각 영역에 번호 레이블
    """
    norm_map = normalize_map(anomaly_map)
    ...
    for cnt in contours:
        ...
        # 반투명 fill
        cv2.drawContours(fill_layer, [cnt], -1, (0, 0, 255), thickness=cv2.FILLED)

        # 외곽선
        cv2.drawContours(result, [cnt], -1, (0, 0, 255), thickness=2)

        # 레이블: contour 무게중심에 번호 표시
        M = cv2.moments(cnt)
        if M["m00"] > 0:
            cx = int(M["m10"] / M["m00"])
            cy = int(M["m01"] / M["m00"])
        ...
        cv2.putText(result, str(label_idx), (cx - 6, cy + 6),
    ...
    # fill 레이어를 원본에 alpha blending
    FILL_ALPHA = 0.25
    result = cv2.addWeighted(fill_layer, FILL_ALPHA, result, 1 - FILL_ALPHA, 0)
...
def save_result_image(
    ...
    # ── 바운딩박스
    bbox_img, defect_regions = draw_defect_contour(
        orig_bgr, anomaly_map, threshold=CONTOUR_THRESHOLD
    )
    ...
    panel_bbox  = add_header(bbox_img,    f"Contour({len(defect_regions)}) | {status}  Score:{pred_score:.3f}")
...
def run_inference(input_path: str, ckpt_path: str, output_dir: str):
    ...
    for pred in predictions:
        ...
        for item in items:
            ...
            # 원본 이미지 로드 후 anomaly_map과 동일한 크기로 맞춤
            # → heatmap / bbox 좌표계를 통일하기 위해 반드시 필요
            orig_bgr = cv2.imread(str(img_path))
            if orig_bgr is None:
                logger.warning(f"  이미지 로드 실패: {img_path}")
                continue
            if anomaly_map is not None:
                map_h, map_w = anomaly_map.shape[-2], anomaly_map.shape[-1]
                orig_bgr = cv2.resize(orig_bgr, (map_w, map_h), interpolation=cv2.INTER_AREA)

            defect_regions = []
            if anomaly_map is not None and (SAVE_HEATMAP or SAVE_CONTOUR):
